...
int getinterval (data&dat, int p0);
int getshape (data&dat,int p0);
// main procedure
...
inline double equalradius(data&dat, int p0)
	{
		double r= dat.intervals[getinterval(dat,p0)].equalradius[getshape(dat,p0)];
...
inline int getinterval(data&dat, int p0)
	{
		double interval = abs(int(dat.particles.Particles[p0]->Tag/dat.numbershapes));
		return (interval);
	}
inline int getshape(data&dat, int p0)
	{
		double shape = abs(dat.particles.Particles[p0]->Tag)%dat.numbershapes;
		return (shape);
	}
// procedure
